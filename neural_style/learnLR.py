...
def train(args):
    ...
    train_dataset = utils.trainingFolder(args.dataset,args.HR_size,LR_scale=None,
                                         motion_len=args.motion_len, motion_angel=args.motion_angel, gauss=args.gauss,
                                         transform=transform,target_transform=transform,mode=args.mode)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size)

    # load validate folder
    val_dataset = utils.trainingFolder(args.valset,args.HR_size,LR_scale=None,
                                       motion_len=args.motion_len, motion_angel=args.motion_angel, gauss=args.gauss,
                                       transform=transform,target_transform=transform,mode=args.mode)
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size)

    transformer = TransformerNet(args.mode,resblock_num=args.resblobk_num,IN_flag = True,deconv_mode=args.deconv_mode)
    ...
    # 读取HR用来测试和看
    if args.mode is "Y":
        ref_HR_y,ref_HR_b,ref_HR_r = utils.load_HR_image(args.style_image, size=None, mode='RGB').split()
        ref_HR = ref_HR_y
    else:
        ref_HR = utils.load_HR_image(args.style_image, mode=args.mode)
        ref_HR_b = None
        ref_HR_r = None

    # 用来测试的HR
    ref_HR = transform(ref_HR)
    if args.cuda:
        ref_HR = ref_HR.cuda()
        transformer.cuda()
    ref_HR = Variable(ref_HR.unsqueeze(0),requires_grad=False)
    if args.normalizebatch_flag is True:
        ref_HR = utils.normalize_batch(ref_HR)

    # 用来看的HR
    if args.mode is not "L":
        ref_HR_RGB = transform(utils.load_HR_image(args.style_image, mode='RGB'))
        ref_LR_RGB = transform(utils.load_LR_image(args.style_image,
                                                   motion_len=args.motion_len,motion_angel=args.motion_angel,gauss=args.gauss,
                                                   scale=None, mode='RGB'))
    else:
        ref_HR_RGB = transform(utils.load_HR_image(args.style_image, mode='L'))
        ref_LR_RGB = transform(utils.load_LR_image(args.style_image,
    ...
    HR_win = viz.image(
        ...
        opts=dict(title='ref img', caption='HR')
    )
    LR_win = viz.image(
        ref_LR_RGB.squeeze(0),
        opts=dict(title='ref img', caption='LR')
    )
    gen_win = viz.image(
        ref_HR_RGB.squeeze(0),
        opts=dict(title='ref img', caption='gen LR')
    )
    ...
    for e in range(args.epochs):
        ...
        for batch_id, (x_HR, x_LR) in enumerate(train_loader):
            ...
            x_gen = transformer(x_HR)


            if args.normalizebatch_flag is True:
                x_gen = utils.unnormalize_batch(x_gen).clamp(0,255)

            content_loss = args.content_weight * lossfun(x_gen, x_LR)
            ...
            if (batch_id + 1) % args.plot_interval == 0:
                ...
                if bestloss is None:
                    bestloss = val_loss
                elif bestloss > val_loss and (e>1 or batch_id>1000):
                    transformer.eval()
                    bestloss = val_loss
                    if args.cuda:
                        transformer.cpu()
            ...
            if args.checkpoint_model_dir is not None and (batch_id + 1) % args.log_interval == 0:
                transformer.eval()
                img = transformer(ref_HR)
                if args.normalizebatch_flag is True:
                    img = utils.unnormalize_batch(img)
                img = draw.var2imgnp(img)
                ref_gen = draw.imgnp_convert(img,args.mode,Cb=ref_HR_b,Cr=ref_HR_r)
                viz.image(
                    ref_gen,
                    win=gen_win,
                    opts=dict(title='ref img', caption='generate LR: '+args.mode)
                )
                transformer.train()
...
def validate(args,net,val_loader,loss_function):
    ...
    for batch_id, (v_HR, v_LR) in enumerate(val_loader):
        ...
        content_loss = args.content_weight * loss_function(v_gen, v_LR)
...
def main():
    ...
    args.deconv_mode = "US"
    args.resblobk_num = 10
    args.model_name = 'LRnet'
    args.lossfun="mse"
