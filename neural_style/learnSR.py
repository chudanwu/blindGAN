...
def train(args):
    ...
    for e in range(args.epochs):
        ...
        for batch_id, (x_HR, x_LR) in enumerate(train_loader):
            ...
            l1reg_loss = torch.abs(l1reg(x_gen)-l1reg(x_HR))
            l1edgereg_loss = torch.abs(l1edgereg(x_gen)-l1edgereg(x_HR))

            totalloss = content_loss  + l1reg_loss * 1e-2 + l1edgereg_loss * 1e-1
            ...
            if (batch_id + 1) % args.log_interval == 0:
                mesg = (
                "{}\tEpoach {}:\t[{}/{}]\t" + args.lossfun + "loss: {:.6f}\t tvloss: {:.6f}\t l1reg: {:.6f}\t l1 edge reg:{:.6f}").format(
                    time.ctime(), e + 1, count, len(train_dataset), content_loss.data[0] / n_batch, tv_loss.data[0]/n_batch,
                    l1reg_loss.data[0]/ n_batch, l1edgereg_loss.data[0]/ n_batch
                )
                print(mesg)
