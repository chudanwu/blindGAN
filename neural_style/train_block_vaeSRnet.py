...
class blindVAE():
    ...
    def __init__(self, opt,isTrain=True,isSeperate=True,fromPretrain=False):
        ...
        self.opt = opt
        if opt.cudaid is not None:
            torch.cuda.set_device(opt.cudaid)

        # define tensor type in/not in cuda
        self.Tensor = torch.cuda.FloatTensor if opt.cudaid else torch.FloatTensor
        self.tensorLR = self.Tensor(opt.batch_size,opt.channelofimg,
        ...
        self.netSR = models.condition_SRNet(opt.param_num,channelofimg=opt.channelofimg,resblock_num=opt.SR_block_num,
                                           norm_mode=opt.SR_norm_mode,drop_out=opt.dropout,
                                           deconv_mode='US',bottle_scale=opt.SR_bottle_scale)
        self.netP = models.condition_LRNet(opt.param_num,channelofimg=opt.channelofimg,resblock_num=opt.P_block_num,
    ...
    def get_current_visuals(self):
        ...
        labeldict = OrderedDict([('lr', lr), ('hr', hr), ('sr', sr)])
        for p in range(self.opt.param_num):
            labeldict[self.opt.params[p] + '_real'] = tensor2im(self.tensorP[:, p, :, :].unsqueeze(1))
            labeldict[self.opt.params[p] + '_fake'] = tensor2im(self.fake_p.data[:, p, :, :].unsqueeze(1))
        return labeldict
    ...
    def save_network(self,epoch,step,label='train'):
        ...
        if label is 'train':
            file_name = combine_or_seperate + 'e{}_st{}_'.format(epoch,step) + 'block_'+file_name
        else:
            file_name = combine_or_seperate + 'best_'.format(epoch, step) + 'block_'+file_name
...
def train(isTrain=True,isSeperate=True,fromPretrain=False,noparams=False):
    ...
    if noparams:
        ...
        from dataFolder import patch_matlab3blur_Folder
        randomblur_data = patch_matlab3blur_Folder(opt)
    ...
    visualizer = draw.Visualizer(model.name(),id=100)
    ...
    if fromPretrain:
        model.load_network("/home/wcd/Desktop/to_neural_style/ckpt/non_params/block4/sepe18_st1440000_nop_block_vaeSR_gsig_3.0_glen_3_mang_89_mlen8_SRIN_block4_PBN_block8.model")
    for epoch in range(1, opt.niter + opt.niter_decay + 1):
        epoch_start_time = time.time()
        for i, data in enumerate(randomblur_loader):
            if len(data[0]) is not opt.batch_size:
...
if __name__ == "__main__":
    #as normal:
    train(isTrain=True,isSeperate=True,fromPretrain=False,noparams=False)
    #no params:
    #train(isTrain=True, isSeperate=True, fromPretrain=False, noparams=True)