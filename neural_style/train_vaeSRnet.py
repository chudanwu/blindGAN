...
class blindVAE():
    ...
    def __init__(self, opt,isTrain=True,isSeperate=True,fromPretrain=False):
        ...
        if self.fromPretrain : # or opt.continue_train:
            self.load_network(self.netSR,'SR',opt.SRmodelname)
            self.load_network(self.netP,'LR',opt.LRmodelname)
    ...
    def set_input(self, input): #LR,HR,P
        input_hr = input[0]
        input_lr = input[1]
        input_p = input[2]
    ...
    def create_condition(self, batch_params):  # batch,params
        if self.opt.batch_size is not batch_params.shape[0]:
            print('error!:batch size is not consistance with patch-params')
        condition_param = torch.zeros((self.opt.batch_size, self.opt.param_num, self.opt.img_size, self.opt.img_size))
        for i in range(batch_params.shape[0]):  # in each batch
            ...
            condition_param[i] = torch.from_numpy(np.array([gauss, motion_x, motion_y])).unsqueeze(1).unsqueeze(
                1).expand(self.opt.param_num, self.opt.img_size, self.opt.img_size).contiguous()
...
model = blindVAE(opt,isTrain=True,isSeperate=True,fromPretrain=False)
